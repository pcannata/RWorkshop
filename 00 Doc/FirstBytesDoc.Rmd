...
Dataframes can be loaded from databases, CSVs, Excel, etc., loading dataframes from these sources will not be covered in this workshop.
...
**You should now be able to open FirstBytesRepo/00 Doc/4diamonds.png. It should look like the following plot.**
...
####A comprehensive KnitR example (which generated this document) can be found in "00 Doc/FirstBytesDoc.Rmd".  
...
To run the shiny app that's in the 06Shiny directory run the following in the main FirstBytesRepo directory (make sure the working directory is set to this directory):  
library(shiny)  
